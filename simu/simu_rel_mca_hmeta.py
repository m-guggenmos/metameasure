...
# Separate script specifically for Mratio based on hmeta-d'
...
for i, nsamples in enumerate(nsamples_list):
    ...
    for j, sigma_sens in enumerate(sigma_sens_list):
        ...
        stimulus_test, choices_test, correct_test, confidence_test, confidence_disc_test, nratings_test = \
            gen_data(nsubjects, nsamples, sigmas_sens=sigma_sens, sigma_meta=sigma_meta_subject, mu=mu, noise_model='beta')
        ...
        mratio_hmeta_test = np.array(type2_SDT_MLE_group(stimulus_test, choices_test, confidence_disc_test, nratings_test).M_ratio)[0]
        mratio_hmeta_retest = np.array(type2_SDT_MLE_group(stimulus_retest, choices_retest, confidence_disc_retest, nratings_retest).M_ratio)[0]
        ...
        df.loc[cond, 'mratio_hmeta_nmae'] = np.mean(np.abs(mratio_hmeta_test_zb - mratio_hmeta_retest_zb)) / \
    ...
    print(f'\t[iter={iter}] Finished nsamp {i + 1} / {nsamples_list_len}: {default_timer() - t0:.1f} secs')

df.to_pickle(f'../data/hmeta_rel/{Path(__file__).stem}_{noise_model}_{iter:03g}.pkl', protocol=4)
