...
def pearsonr_ci(x, y, alpha=0.05):
    ...
    Parameters
    ----------
    x, y : iterable object such as a list or array
    ...
    # SE = (hi - lo) / (2 * 1.96)
    # For the below SE equation, see: Cohen, J., & Cohen, J. (Eds.). (2003).
    # Applied multiple regression/correlation analysis for the behavioral sciences (3rd ed). Mahwah, N.J:
...
def plot_coefficients(model, data, exclude_categorical=False, colors_reg=None, colors_corr=None,
                      ...
                      asterisk_yshift=0, circle_yshift=0, asterisk_fontsize=12, circle_fontsize=14, mapping=None,
                      min_x_asterisks=-np.inf, max_x_asterisks=np.inf, show_trend=True):
    ...
    if exclude_categorical:
        param_names = [p for p in params.index if not p.startswith('C(') and not p.endswith(' Var')]
        params = params[[p for p in params.index if not p.startswith('C(') and not p.endswith(' Var')]]
    ...
    for i, p in enumerate(param_names):
        ...
        r[i] = corrstats.r  # noqa
        r_pval[i] = corrstats.p  # noqa
        r_se[i] = corrstats.SE  # noqa
    ...
    if striped_background:
        for i in range(nparams+1):
            plt.axhspan(i-0.5, i+0.5, facecolor=[stripe_color_light, stripe_color_dark][int(np.mod(i, 2) == 0)],
                        alpha=0.1)

    for i, (param, val) in enumerate(params.iteritems()):

        plt.barh(nparams - i - 1 + 0.25, r[i], fc=colors_corr[i], xerr=r_se[i], height=0.5,
                 label='Bivariate\ncorrelation' if i == 0 else None)
        plt.barh(nparams - i - 1 - 0.25, val, fc=colors_reg[i], xerr=se[i], height=0.5,
        ...
        if (r_pval[i] < 0.1) and not ((r_pval[i] >= 0.05) and not show_trend):
            ...
            plt.text((max(r[i]-r_se[i]-0.01, min_x_asterisks), min(r[i]+r_se[i]+0.01, max_x_asterisks))[int(r[i] > 0)],
                     nparams - i + 0.25 - 1 - 0.22 + yshift + 0.3*(((r[i]-r_se[i]+0.01) > max_x_asterisks) | ((r[i]-r_se[i]+0.01) < min_x_asterisks)),  # noqa
                     asterisks, ha=('right', 'left')[int(r[i] > 0)], fontsize=fontsize, fontweight=boldfont)
        if (pval[i] < 0.1) and not ((pval[i] >= 0.05) and not show_trend):
            ...
            plt.text((max(val-se[i]-0.01, min_x_asterisks), min(val+se[i]+0.01, max_x_asterisks))[int(val > 0)],
                     nparams - i - 1 - 0.25 - 0.22 + yshift + 0.3*(((val+se[i]+0.01) > max_x_asterisks) | ((val+se[i]+0.01) < min_x_asterisks)),  # noqa
                     asterisks, ha=('right', 'left')[int(val > 0)], fontsize=fontsize, fontweight=boldfont)
