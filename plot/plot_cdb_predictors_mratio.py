...
z = z[z.study_id.isin(np.where((z.groupby('study_id').nsamples.mean() >= N).values)[0])]
...
for i, modus in enumerate(('correlation', 'nmae')):
    ...
    ps = ['d1', 'conf_norm'] + dbvars
    model = linear_regression(
        zstudy[~zstudy['mratio'].isna()],
        patsy_string=f'mratio ~ ' + ' + '.join(ps),
        model_blocks=False,
        ols=True,
    ...
    plot_coefficients(model, zstudy[~zstudy[f'{measure}_reliability'].isna()], exclude_categorical=True, striped_background=True, asterisk_yshift=-0.015, asterisk_fontsize=9, circle_fontsize=11,
                      circle_yshift=-0.13, mapping=ps_mapping, max_x_asterisks=0.95, stripe_color_dark=(0.15, 0.15, 0.15), show_trend=True)
